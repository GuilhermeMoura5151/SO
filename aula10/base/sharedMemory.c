...
{
  return shmget ((key_t) key, size, MASK | IPC_CREAT | IPC_EXCL); // criar/aceder memoria partilhada
}
...
{
  return shmctl (shmid, IPC_RMID, (struct shmid_ds *) NULL); // controlar memoria partilha  da (remover memoria partilhada etc)
}
...
int shmemAttach (int shmid, void **pAttAdd)
{
  ...
  add = shmat (shmid, (char *) NULL, 0); // shmat - attach *** shmid eh o returno de shmget
  if (add != (void *) -1)
     { *pAttAdd = (void *) add;
       return 0;
     }
...
{
  return shmdt (attAdd); // shmdt - detach
}
